 ...
 */
static void response_ok() { uart_putsln("-> OK"); }
...
static int8_t stoui(const char *s, uint8_t *i)
{
	...
	if (digits[0] < 0 || 9 < digits[0] || digits[1] < 0 || 9 < digits[1] ||
...
static void handle_help(void)
{
	...
	uart_putsln(
	    "gotoxy <0-15> <0-1>   go's to the given coordinates on the lcd");
	uart_putsln("clearline             clears the current line where the "
		    "cursor is located");
	uart_putsln("write <content>       writes the given content to the "
		    "current position");
	uart_putsln("--------------------------");
}
...
void interprete(const char *content)
{
	...
	if (starts_with_command(content, CMD_HELP)) {
		...
		response_error(
		    "unkown command! type help to see a list of commands!");
	}
}
